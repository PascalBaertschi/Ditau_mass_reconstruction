...
histditaumassregall = ROOT.TH1D("ditaumassregall","reconstruct di-#tau mass using visible di-#tau parameters",100,0,100)
#histditaumassregall.GetXaxis().SetTitle("")
histditaumassregall.GetXaxis().SetTitle("di-#tau_{gen} mass [GeV]")
...
histditaumassregallratio.GetXaxis().SetTitle("di-#tau_{gen} mass [GeV]")
...
histditaumassregtotalout = ROOT.TH1D("ditaumassregtotalout","reconstruct di-#tau mass using di-#tau 4-vector",100,0,100)
histditaumassregtotalout.GetXaxis().SetTitle("di-#tau_{gen} mass [GeV]")
histditaumassregtotalout.GetYaxis().SetTitle("number of occurence")
histditaumassregtotalout.SetLineColor(2)
histditaumassregtotalout.SetLineWidth(3)
histditaumassregtotalout.SetStats(0)
histditaumassregtotal = ROOT.TH1D("ditaumassregtotal","reconstruct di-#tau mass using di-#tau 4-vector",100,0,100)
histditaumassregtotal.SetLineColor(4)
histditaumassregtotal.SetLineWidth(3)
histditaumassregtotal.SetLineStyle(7)
...
histditauregthetaout = ROOT.TH1D("ditauregthetaout","reconstruct di-#tau #theta using di-#tau 4-vector",100,0,3.5)
histditauregthetaout.GetXaxis().SetTitle("di-#tau_{gen} #theta")
histditauregthetaout.GetYaxis().SetTitle("number of occurence")
histditauregthetaout.SetLineColor(2)
histditauregthetaout.SetLineWidth(3)
histditauregthetaout.SetStats(0)
histditauregtheta = ROOT.TH1D("ditauregtheta","reconstruct di-#tau #theta using di-#tau 4-vector",100,0,3.5)
histditauregtheta.SetLineColor(4)
histditauregtheta.SetLineWidth(3)
histditauregtheta.SetLineStyle(7)
histditauregtheta.SetStats(0)


#histogram of ditau vismass with regression all particles with 4Vector (all decay-products)
histditauvismassregallout = ROOT.TH1D("ditauvismassregallout","reconstruct visible di-#tau mass using visible di-#tau 4-vector",100,0,100)
histditauvismassregallout.GetXaxis().SetTitle("visible di-#tau_{gen} mass [GeV]")
histditauvismassregallout.GetYaxis().SetTitle("number of occurence")
histditauvismassregallout.SetLineColor(2)
histditauvismassregallout.SetLineWidth(3)
histditauvismassregallout.SetStats(0)
histditauvismassregall = ROOT.TH1D("ditauvismassregall","reconstruct visible di-#tau mass using visible di-#tau 4-vector",100,0,100)
histditauvismassregall.SetLineColor(4)
histditauvismassregall.SetLineWidth(3)
histditauvismassregall.SetLineStyle(7)
...
def mass_model_total(batch_size,epochs,output_name):
    ...
    mass_model_total.add(Dense(5,kernel_initializer='random_uniform',activation='relu'))
    #mass_model_total.add(Dropout(0.2))
    #mass_model_total.add(BatchNormalization()))
    ...
    """
    for i in range(0,len(test_input_total[:,0])):
    ...
    max_bin1 = max(histditaumassregtotal.GetMaximum(),histditaumassregtotalout.GetMaximum())
    histditaumassregtotalout.SetMaximum(max_bin1*1.08)
    histditaumassregtotalout.Draw()
    histditaumassregtotal.Draw("SAME")
    leg = ROOT.TLegend(0.65,0.7,0.87,0.87)
    leg.AddEntry(histditaumassregtotalout,"di-#tau_{gen} mass","PL")
    leg.AddEntry(histditaumassregtotal,"di-#tau mass","PL")
...
def theta_model(batch_size,epochs,output):
    ...
    histditauregthetaout.SetMaximum(max_bin*1.3)
    histditauregthetaout.Draw()
    histditauregtheta.Draw("SAME")
    leg = ROOT.TLegend(0.65,0.7,0.87,0.87)
    leg.AddEntry(histditauregthetaout,"di-#tau_{gen} #theta","PL")
    leg.AddEntry(histditauregtheta,"di-#tau #theta","PL")
    leg.SetTextSize(0.04)
    leg.Draw()
...
def vismass_model_all(batch_size,epochs,output):
    ...
    vismass_model_all.fit(train_input_vismass_all,train_output_vismass_all,batch_size,epochs,verbose=2)
    ...
    max_bin = max(histditauvismassregall.GetMaximum(),histditauvismassregallout.GetMaximum())
    histditauvismassregallout.SetMaximum(max_bin*1.08)
    histditauvismassregallout.Draw()
    histditauvismassregall.Draw("SAME")
    leg = ROOT.TLegend(0.55,0.7,0.87,0.87)
    leg.AddEntry(histditauvismassregallout,"visible di-#tau_{gen} mass","PL")
    leg.AddEntry(histditauvismassregall,"visible di-#tau mass","PL")
    leg.SetTextSize(0.04)
    leg.Draw()
    output_name = "%s.png" %(output)
    img = ROOT.TImage.Create()
...
def mass_model_all(batch_size,epochs,learning_rate,decay_rate,output_name):
    ...
    leg2 = ROOT.TLegend(0.6,0.7,0.87,0.87)
    leg2.AddEntry(histditaumassregallout,"di-#tau_{gen} mass","PL")
    leg2.AddEntry(histditaumassregall,"di-#tau mass","PL")
...
output_name = "ditau_mass_4Vector_45"
#output_name = "ditau_mass_without_nupz_58"
#output_name = "ditau_mass_4Vector_vismass18"
#output_name = "ditau_mass_4Vector_theta19"
description_of_training = "INPUT:vis di-tau 4vec+nu px+nu py"
...
#mass_model_all(70,100,learning_rate,decay_rate,output_name)
mass_model_total(70,100,output_name)
